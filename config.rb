...
if ARGV[0].eql?('up')
    ...
    $new_discovery_url="https://discovery.etcd.io/new?size=#{$num_master_nodes+$num_worker_nodes}"
    $discovery_token = open($new_discovery_url).read

    ["master", "worker"].each do |type|
        user_data_filename = "user-data/#{type}"
        if File.exists?(user_data_filename)
            data = YAML.load(IO.readlines(user_data_filename)[1..-1].join)
            ...
            if data.key? 'coreos' and data['coreos'].key? 'etcd2'
                data['coreos']['etcd2']['discovery'] = $discovery_token
            end
            ...
            yaml = YAML.dump(data)
            File.open(user_data_filename, 'w') { |file| file.write("#cloud-config\n\n#{yaml}") }
        end
    end
end
